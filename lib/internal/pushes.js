...
PushBullet.prototype.file = async function file(deviceParams, filePath, body) {
	...
	var uploadRequestResponse = await fetch(PushBullet.UPLOAD_END_POINT, {
		method: 'post',
		body: JSON.stringify({
			file_name: fileName,
			file_type: fileType
	...
	var uploadRequestResponseJson = await uploadRequestResponse.json();

	var formData = new FormData();
	formData.append('file', fs.createReadStream(filePath));

	var uploadFileResponse = await fetch(uploadRequestResponseJson.upload_url, {
		method: 'post',
		body: formData
	});
