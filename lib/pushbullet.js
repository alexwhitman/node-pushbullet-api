 ...
 *
 * @param  {String}   deviceId Device ID of device to push to.
 ...
 */
PushBullet.prototype.note = function note(deviceId, title, body, callback) {
	this.push({
		device_id: deviceId,
		type: 'note',
		title: title,
 ...
 */
PushBullet.prototype.address = function address(deviceId, name, body, callback) {
	this.push({
		device_id: deviceId,
		type: 'address',
		name: name,
 ...
 * @param  {Function} callback Callback for when the request is complete.
 */
PushBullet.prototype.list = function list(deviceId, title, items, callback) {
 ...
 * @param  {Function} callback Callback for when the request is complete.
 */
PushBullet.prototype.link = function link(deviceId, title, url, callback) {
	this.push({
		device_id: deviceId,
		type: 'link',
		title: title,
 ...
 * @param  {Function} callback Callback for when the request is complete.
 */
PushBullet.prototype.file = function file(deviceId, filePath, callback) {
	this.push({
		device_id: deviceId,
		type: 'file',
		file: filePath
...
PushBullet.prototype.push = function push(bullet, callback) {
	...
	if (bullet.type === 'file') {
		var form = req.form();
		form.append('device_id', bullet.device_id);
