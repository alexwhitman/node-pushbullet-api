...
PushBullet.prototype.history = function history(options, callback) {
	...
	if (options.cursor) {
		parameters.qs.cursor = options.cursor;
	}

	self.request.get(PUSH_END_POINT, parameters, function(error, response, body) {
		self.handleResponse(error, response, body, callback);
	});
};
...
PushBullet.prototype.devices = function devices(callback) {
	...
	self.request.get(DEVICES_END_POINT, function(error, response, body) {
		self.handleResponse(error, response, body, callback);
	});
};
...
PushBullet.prototype.contacts = function contacts(callback) {
	...
	self.request.get(CONTACTS_END_POINT, function(error, response, body) {
		self.handleResponse(error, response, body, callback);
	});
};
