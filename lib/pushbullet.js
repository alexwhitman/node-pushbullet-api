...
const Encryption = require('./internal/encryption');
...
PushBullet.prototype.makeRequest = function makeRequest(verb, endPoint, options, callback) {
	return new Promise((resolve, reject) => {
		this.request[verb](endPoint, options).then(response => {
			...
			this.handleError(err, null, callback);
			reject(err);
		});
	});
}
...
PushBullet.prototype.handleError = function handleError(error, response, callback) {
	return new Promise((resolve, reject) => {
		...
		if (response.statusCode !== 200) {
			const err = response.body.error && response.body.error.message ? new Error(response.body.error.message) : new Error(http.STATUS_CODES[response.statusCode])
